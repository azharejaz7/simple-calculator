...
while (attempLogin != 0) {
    ...
    if (atmPin === answer.ChoosePin) {
      ...
      if (answer.selectedOption === "Withdraw") {
        ...
        if (balanceAmt >= answer.withdrawAmt) {
          ...
          console.log(
            `Sorry! your withdraw amount is exceeding from your Current Balance.`
          );
        }
      } else {
  ...
  if (attempLogin === 0) {
    console.log(
      `Sorry! your account is temporary blocked by System for security purpose.. `
    );
  }
}
